...
extension JTAppleCalendarView {
    ...
    public func visibleDates(_ completionHandler: @escaping (_ dateSegmentInfo: DateSegmentInfo) ->()) {
        ...
        let rect = CGRect(
          x: calendarView.contentOffset.x + 1,
          y: calendarView.contentOffset.y + 1 + calendarView.contentInset.top,
          width: calendarView.frame.width - 2,
          height: calendarView.frame.height - 2 - calendarView.contentInset.top - calendarView.contentInset.bottom
        )
            ...
            if retrievedPathsFromDates.count > 0 {
                ...
                if self.scrollingMode.pagingIsEnabled() {
                    if self.registeredHeaderViews.count > 0 {
                        ...
                        // then scroll to the first day of the week of that row
                      if self.numberOfRows() == 1 &&
                        self.cachedConfiguration.generateInDates == .forFirstMonthOnly &&
                        self.cachedConfiguration.generateOutDates == .off {
                        // Fix just for single row calendar
                        let scrollingToCell = self.cellStateFromIndexPath(sectionIndexPath)
                        let scrollingToDayIndex = scrollingToCell.day.rawValue
                        let firstDayOfWeekIndex = self.cachedConfiguration.firstDayOfWeek.rawValue
                        let offsetDayIndex = scrollingToDayIndex - firstDayOfWeekIndex
                        
                        var findFirstDayOfWeekIndex = { (offsetDayIndex: Int, currentIndexPath: IndexPath) -> IndexPath in
                          var firstDayOfWeekIndexPath = currentIndexPath
                          firstDayOfWeekIndexPath.item = currentIndexPath.item - offsetDayIndex
                          while(firstDayOfWeekIndexPath.item < 0 && firstDayOfWeekIndexPath.section > 0) {
                            firstDayOfWeekIndexPath.section -= 1
                            
                            firstDayOfWeekIndexPath.item += self.numberOfItemsInSection(firstDayOfWeekIndexPath.section)
                          }
                          
                          return firstDayOfWeekIndexPath
                        }
                        
                        let currentIndexPath = sectionIndexPath
                        var firstDayOfWeekIndexPath = findFirstDayOfWeekIndex(offsetDayIndex, currentIndexPath)
                        
                        scrollToIndexPath(firstDayOfWeekIndexPath, animateScroll)
                      } else {
                        // for the normal case
                        scrollToIndexPath(
                          IndexPath(
